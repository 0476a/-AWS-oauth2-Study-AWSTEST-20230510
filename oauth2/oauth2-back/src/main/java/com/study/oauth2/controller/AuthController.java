...
public class AuthController {
	...
	public ResponseEntity<?> providerMerge(@RequestBody OAuth2ProviderMergeReqDto auth2ProviderMergeReqDto) {
		
		if(!authService.checkPassword(auth2ProviderMergeReqDto.getEmail(), auth2ProviderMergeReqDto.getPassword())) {
			return ResponseEntity.badRequest().body("비밀번호가 일치하지 않습니다");
		}
		
		return ResponseEntity.ok(authService.oAuth2ProviderMerge(auth2ProviderMergeReqDto));
	}
